...
- (instancetype)initWithFrame:(CGRect)frame forcusImages:(NSArray *)images titles:(NSArray *)titles
{
    ...
    if (self) {
        ...
        //        pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:0.937 green:0.286 blue:0.286 alpha:1.000];
        ...
        UILabel * countLbl = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, WH, WH)];
        countLbl.center = CGPointMake(CGRectGetWidth(frame) -WH/2 -15, CGRectGetHeight(frame) -WH/2 -15);
        countLbl.layer.cornerRadius = WH/2;
        countLbl.layer.masksToBounds = YES;
        countLbl.font = [UIFont systemFontOfSize:18.0f];
        countLbl.textAlignment = NSTextAlignmentCenter;
        countLbl.textColor = [UIColor whiteColor];
        countLbl.backgroundColor = [UIColor colorWithWhite:0.000 alpha:0.545];
        countLbl.text = [NSString stringWithFormat:@"1/%ld",images.count];
        
        [self addSubview:countLbl];
        self.countLbl = countLbl;
        //自动缩放字体
        self.countLbl.adjustsFontSizeToFitWidth = YES;
        self.countLbl.baselineAdjustment = UIBaselineAdjustmentAlignCenters;
...
-(void)tapToImage
{
    ...
    {
        [self.delegate focusImageWithtouchImagePage:(self.currentPage -1) imageurl:self.images[self.currentPage-1] scrollView:self.scrollView];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    self.countLbl.text = [NSString stringWithFormat:@"%ld/%ld",MAX(1, MIN(page, self.imageCounts)),self.imageCounts];
