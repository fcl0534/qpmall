...
@interface CarTypesViewController ()<UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate>
...
@implementation CarTypesViewController
...
- (void)fetchLevel1{
    ...
    fetchLevel1 = [fetchLevel1 stringByAppendingFormat:@"userId=%@",self.userId];

    [self.NetworkUtil GET:fetchLevel1 inView:self.view success:^(id responseObject) {
...
- (void)initTableView{
    ...
    searchBar.tintColor = self.mainColor;
    ...
    self.listTableView =
    [[UITableView alloc] initWithFrame:CGRectMake(0, 64 +45, K_UIScreen_WIDTH,
                                                  self.isFromOther ? K_UIScreen_HEIGHT -64 -45:K_UIScreen_HEIGHT -64 -49 -45) style:UITableViewStylePlain];
...
//        [self requestVINInfo:key];
        ...
        [dic setValue:key forKey:KEY_searchId];
        [dic setValue:key forKey:KEY_searchName];
...
- (void)requestVINInfo:(NSString *)key {

    NSString *url=[API_ROOT stringByAppendingString:API_GOODS_LIST];

    url = [url stringByAppendingFormat:@"&vin=%@",key];
    ...
    [self.NetworkUtil GET:url header:header inView:self.view success:^(id responseObject) {
        ...
        if ([[responseObject objectForKey:@"status"] integerValue] == 1) {
            ...
            if (goodsArray.count > 0) {
                ...
                filterName = [dataDic objectForKey:@"car_mode_name"]?[dataDic objectForKey:@"car_mode_name"]:key;
            }
...
- (void)fetchLevel3With:(NSIndexPath *)indexPath{
    ...
    [self.NetworkUtil GET:searchListUrl inView:self.view success:^(id responseObject) {
        ...
        CarType2ViewController *vc = [[CarType2ViewController alloc]init];
        ...
        vc.fetchUrl = searchListUrl;//[searchListUrl stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        [self.navigationController pushViewController:vc animated:YES];
