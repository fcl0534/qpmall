...
@interface QPMOrderConfirmViewController ()<
UITableViewDataSource,
UITableViewDelegate,
PaymentAddressViewDelegate,
PaymentSubmitBarViewDelegate,
PaymentFooterViewDelegate>
...
@implementation QPMOrderConfirmViewController
...
//    return;
    ...
    btn.enabled = NO;
    NSString *cartSelectAllUrl=[API_ROOT stringByAppendingString:API_ORDER_CREATE];
    ...
    if (self.paymetType == PaymentTypeOfflinepay) {
...
//    }
    ...
    [self.NetworkUtil POST:cartSelectAllUrl header:header parameters:parameters inView:self.view success:^(id responseObject) {
        ...
        if ([[responseObject objectForKey:@"status"] intValue] == 1) {
            ...
            [infoMDic setObject:payPrice forKey:@"payPrice"];
            [infoMDic setObject:[data objectForKey:@"transactionId"] forKey:@"transactionId"];
            ...
            switch (self.paymetType) {
                case 1: {
                    ...
                    vc.infoDic = infoMDic;
                    vc.paymentSubmitSource = QPMPaymentSubmitSourceCart;
                    [self.navigationController pushViewController:vc animated:YES];
