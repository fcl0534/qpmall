...
-(id)initWithSize:(CGSize)size rootViewController:(YQViewController *)viewController{
    ...
    if(self=[super init]){
        ...
        self.containerBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        ...
        self.containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 64, size.width, size.height-64)];
...
-(id)initWithFrame:(CGRect)frame rootViewController:(YQViewController *)viewController{
    ...
    if(self=[super init]){
        ...
        self.containerBackgroundView = [[UIView alloc] initWithFrame:frame];//CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        self.containerBackgroundView.backgroundColor = [UIColor clearColor];
...
//        self.containerBackgroundView.clipsToBounds = YES;
        ...
        self.containerView = [[UIView alloc] initWithFrame:CGRectMake(0, +64, frame.size.width, frame.size.height-64)];
        self.containerView.backgroundColor = [UIColor groupTableViewBackgroundColor];
        [self.containerBackgroundView addSubview:self.containerView];
...
-(void)panHandle:(UIPanGestureRecognizer *)gesture{
    if(self.panPopView && self.viewControllers.count>1){
        ...
        if(gesture.state == UIGestureRecognizerStateBegan){
            ...
            if(flag>0 && flag<self.frame.size.width){
                self.tempToVC.view.center = CGPointMake(self.tempToVC.view.center.x+tran.x*0.5, self.tempToVC.view.center.y);
                self.tempCurrentVC.view.center = CGPointMake(self.tempCurrentVC.view.center.x+tran.x, self.tempCurrentVC.view.center.y);
                CGFloat alpha = self.tempCurrentVC.view.frame.origin.x/self.frame.size.width;
                self.tempToVC.navigationBar.titleAlpha = alpha*1.5;
                self.tempCurrentVC.navigationBar.titleAlpha = 1.0-alpha*1.5;
            }
        }else{
            if(self.tempCurrentVC.view.center.x>=self.frame.size.width){
                //过了一半完成pop
                [self.tempCurrentVC willMoveToParentViewController:nil];
                [self addChildViewController:self.tempToVC];
                CGFloat duration = (self.frame.size.width-self.tempCurrentVC.view.frame.origin.x)/self.frame.size.width*0.3;
                [UIView animateWithDuration:duration animations:^{
                    self.tempToVC.view.center = CGPointMake(self.frame.size.width/2, self.tempToVC.view.center.y);
                    self.tempCurrentVC.view.frame = CGRectMake(self.frame.size.width, 0, self.frame.size.width, self.containerView.bounds.size.height);
                ...
                CGFloat duration = self.tempCurrentVC.view.frame.origin.x/self.frame.size.width*0.3;
                [UIView animateWithDuration:duration animations:^{
                    //tempToVC的中心无所谓
...
//                    [self.containerBackgroundView bringSubviewToFront:self.tempCurrentVC.navigationBar];
                    self.tempCurrentVC.view.center = CGPointMake(self.frame.size.width/2, self.tempCurrentVC.view.center.y);
                } completion:^(BOOL finished) {
...
//        ((YQViewController *)self.viewControllers.lastObject).navigationBar.frame = CGRectMake(0, 0, size.width, 64);
//    }
//    
//}
-(void)setFrame:(CGRect)frame{
    ...
    if(self.containerBackgroundView){
        //CGPoint point = self.containerBackgroundView.center;
        self.containerBackgroundView.frame = frame;//CGRectMake(0, 0, frame.size.width, frame.size.height);
        //self.containerBackgroundView.center = point;
        self.containerView.frame = frame;//self.containerBackgroundView.bounds;
        ((YQViewController *)self.viewControllers.lastObject).navigationBar.frame = CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, 64);
    }
}
- (void)viewDidLoad
{
    ...
    //self.containerBackgroundView.frame = self.frame;//self.view.center;
    ...
    if(self.rootViewController){
        ...
        self.rootViewController.navigationBar.frame = CGRectMake(0, 0, self.frame.size.width, 64);
        self.rootViewController.navigationBar.hidden = NO;
        self.rootViewController.navigationBar.tintColor = [UIColor whiteColor];
...
//        viewController.view.userInteractionEnabled = YES;
        ...
        if(animated){
            self.animating = YES;
            toNavigationBar.frame = CGRectMake(self.containerView.bounds.size.width, 0, currentNavigationBar.bounds.size.width, currentNavigationBar.bounds.size.height);
            viewController.view.frame = CGRectMake(self.containerView.bounds.size.width, 0, self.containerView.bounds.size.width, self.containerView.bounds.size.height);
            [self transitionFromViewController:currentViewController toViewController:viewController duration:0.2 options:UIViewAnimationOptionCurveLinear animations:^{
                ...
                currentViewController.view.frame = CGRectMake(0, 0, self.containerView.bounds.size.width, self.containerView.bounds.size.height);
            } completion:^(BOOL finished){
                if(finished){
    ...
    if(self.viewControllers.count>1){
        ...
        if(animated){
            ...
            [self transitionFromViewController:currentViewController toViewController:toViewController duration:0.2 options:UIViewAnimationOptionCurveLinear animations:^{
                ...
                //currentNavigationBar.alpha = 0.0f;
                currentNavigationBar.frame = CGRectMake(self.containerView.bounds.size.width, 0, currentNavigationBar.bounds.size.width, currentNavigationBar.bounds.size.height);
                currentViewController.view.frame = CGRectMake(self.containerView.bounds.size.width, 0, self.containerView.bounds.size.width, self.containerView.bounds.size.height);
