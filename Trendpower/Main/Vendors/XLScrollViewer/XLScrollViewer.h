...
@protocol XLScrollViewerDelegete <NSObject>
...
@property (nonatomic, weak) id<XLScrollViewerDelegete>delegate;

#pragma mark -使用XLScrollView
/**
 *     使用XLScrollView,数组如果填nil，则为默认，choose处填写三位数字，代表选择与否，1代表选择，2代表不选择。例如：你准备选择第1种和第3种动画效果，则填写121
 */
-(instancetype)initWithFrame:(CGRect)frame withViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose selectIndex:(NSInteger)selectIndex;//实例方法
+(instancetype)scrollWithFrame:(CGRect)frame withViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose selectIndex:(NSInteger)selectIndex;//类方法
