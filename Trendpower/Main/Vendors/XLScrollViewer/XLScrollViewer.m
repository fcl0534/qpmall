...
-(instancetype)initWithFrame:(CGRect)frame withViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose selectIndex:(NSInteger)selectIndex{
...
+(instancetype)scrollWithFrame:(CGRect)frame withViews:(NSArray *)views withButtonNames:(NSArray *)names withThreeAnimation:(int)choose selectIndex:(NSInteger)selectIndex{
    return [[self alloc]initWithFrame:frame withViews:views withButtonNames:names withThreeAnimation:choose selectIndex:selectIndex];
}
...
-(void)addAll {
    if ((self.xl_buttonNames.count || self.xl_views.count) && self.xl_buttonNames.count !=self.xl_views.count) {
        ...
        self.xl_buttonNames =self.xl_buttonNames?self.xl_buttonNames:@[@"view1",@"view1",@"view1"];
...
-(void)addScroll1{
    ...
    CGSize size0 =[self.xl_buttonNames[self.defaultIndex] sizeWithAttributes:@{NSFontAttributeName :[UIFont systemFontOfSize:self.xl_buttonFont?self.xl_buttonFont:18]}];
    UIButton *button0 =self.buttons[self.defaultIndex];
    ...
    //底部线条
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.scroll1.frame)-0.5, self.scroll1.frame.size.width,0.5)];
    line.backgroundColor = [UIColor colorWithWhite:0.860 alpha:1.000];
    [self.scroll1 addSubview:line];
...
-(void)changed:(UIButton *)button{
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        ...
        if (temp ==button) {
            [temp setTitleColor:self.xl_buttonColorSelected?self.xl_buttonColorSelected:[UIColor whiteColor] forState:UIControlStateNormal];
            if ([self.delegate respondsToSelector:@selector(XLScrollViewerSelectIdexPath:)]) {
                [self.delegate XLScrollViewerSelectIdexPath:i];
            }
            //NSLog(@"点击---%d",i);
        }else {
            [temp setTitleColor:self.xl_buttonColorNormal?self.xl_buttonColorNormal:[UIColor blackColor] forState:UIControlStateNormal];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (self.xl_isMoveButton) {
        ...
        if (self.xl_buttonNames.count <=5) {
            self.view2.transform =CGAffineTransformMakeTranslation(scrollView.contentOffset.x/self.xl_buttonNames.count -_x0, 0);
        }else {
            self.view2.transform =CGAffineTransformMakeTranslation(scrollView.contentOffset.x/5 -_x0, 0);
        }
    }
...
//                NSArray * sizesToAnimate = [[NSArray alloc] initWithObjects:
//                                  [NSNumber numberWithFloat:21.0],
//                                  [NSNumber numberWithFloat:32.0],
//                                  [NSNumber numberWithFloat:10.0],
//                                  [NSNumber numberWithFloat:17.0], nil];
//                float newSize = [[sizesToAnimate objectAtIndex:1] floatValue];
//                
//                UIFont *font = [UIFont systemFontOfSize:self.xl_buttonFont?self.xl_buttonFont:18];
//                UIFont *newFont = [UIFont fontWithName:font.fontName size:100];
//                temp.titleLabel.font = newFont;
                
            }completion:^(BOOL finished) {
                [temp.titleLabel setFont:[UIFont systemFontOfSize:self.xl_buttonFont?self.xl_buttonFont:18]];
                // 滚动完成时
               // NSLog(@"---%@",temp.currentTitle);
                for (int i = 0; i < self.xl_buttonNames.count; i++) {
                    if ([self.xl_buttonNames[i] isEqualToString:temp.currentTitle]) {
                        if ([self.delegate respondsToSelector:@selector(XLScrollViewerSelectIdexPath:)]) {
                            [self.delegate XLScrollViewerSelectIdexPath:i];
                        }
                        //NSLog(@"滚动完成----%d",i);
