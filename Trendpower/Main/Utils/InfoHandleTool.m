...
#import "InfoHandleTool.h"

UIWebView * _webView;

@implementation InfoHandleTool

+ (instancetype)sharedInfoTool
{
    static InfoHandleTool *infoTool;
    @synchronized(self) {
        if (!infoTool)
...
//    }
    ...
    if ([self.delegate respondsToSelector:@selector(infoHandleToolMessageComposeViewController:didFinishWithResult:)]) {
        [self.delegate infoHandleToolMessageComposeViewController:controller didFinishWithResult:result];
    }
}
...
//    }
    ...
    if ([self.delegate respondsToSelector:@selector(infoHandleToolMailComposeController:didFinishWithResult:error:)]) {
        [self.delegate infoHandleToolMailComposeController:controller didFinishWithResult:result error:error];
    }
 ...
 *  //打开系统拍照机
 */
- (void)openCamera:(UIViewController *)vc square:(BOOL)square
{
    //拍照
    UIImagePickerController * camera = [[UIImagePickerController alloc]init];
    
    camera.sourceType = UIImagePickerControllerSourceTypeCamera;
    camera.allowsEditing = square;    //编辑成正方形
 ...
 *  //打开系统照片库
 */
- (void)openPhotoLibrary:(UIViewController *)vc square:(BOOL)square;
{
    //从相册
...
////    [UIImagePNGRepresentation(newImage) writeToFile:uniquePath atomically:YES];
    ...
    if ([self.delegate respondsToSelector:@selector(infoHandleToolImagePickerController:didFinishPickingMediaWithInfo:)]) {
        [self.delegate infoHandleToolImagePickerController:picker didFinishPickingMediaWithInfo:info];
    }
}
